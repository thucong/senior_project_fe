    ...
    return (
      <div className="col col-lg-8 mt-5 pr-5 ">
        ...
        {consultation.length > 0
          ? consultation.map((item, index) => (
              <div className="row mb-3 consultation p-3" key={index}>
                ...
                <div className="col col-10 info-consultation pl-4">
                  <p>
                    Tên bác sĩ:{" "}
                    <span className="info-item">{item.doctorId.fullname}</span>
                  </p>
                  <p>
                    Ngày: <span className="info-item">{item.date}</span>
                  </p>
                  <p>
                    Thời gian:{" "}
                  ...
                  <p>
                    Lý do của cuộc tư vấn:{" "}
                    <span className="info-item">{item.content}</span>
                  </p>
                  <p>
                    Trạng thái:{" "}
                    <span
                  ...
                  {item.status === "waiting" ? (
                    <div className="mt-2">
                      <button
                        ...
                        onClick={(e) => this.onChoose(item._id)}
                      >
                        Sửa
                      </button>
                      &ensp;
                      <button
                        ...
                        onClick={(e) => this.onChooseDelete(item._id)}
                      >
                        Xóa
                      </button>
                    </div>
                  ...
                  {item.status === "confirmed" ? (
                    <div>
                      <div>
                        <p>
                          Ghi chú:{" "}
                          <span className="info-item">{item.note}</span>
                        </p>
                      </div>
                      {new Date().getTime() < new Date(item.start).getTime()
                        ? this.countdown(item.start)
                        : ""}
                      {new Date().getTime() > new Date(item.end).getTime()
                        ? this.changeStatus(item._id)
                        : ""}
                      {!this.state.isShowCallButton ? (
                        <p>
                          Thời gian còn lại cho đến cuộc tư vấn:{" "}
                          <span className="info-item">
                        ...
                        <button
                          ...
                          onClick={(e) => this.call(item.linkCall)}
                        >
                          Bắt đầu cuộc gọi
                        </button>
                      )}
                  ...
                  {item.status === "reject" ? (
                    <p>
                      Lý do từ chối:{" "}
                      <span className="info-item">{item.reasonOfReject}</span>
                    </p>
                  ) : (
                    ""
                  )}
          ...
          : ""}
        <EditConsultation
          info_consultation={this.state.info_consultation}
